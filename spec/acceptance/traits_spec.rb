describe "an instance generated by a factory with multiple traits" do
    ...
    FactoryBot.define do
      ...
      factory :post do
        ...
        factory :female, traits: [:female] do
          ...
          factory :female_draft_judy, traits: [:draft]
        end

        factory :female_draft,            traits: [:female, :draft]
        factory :female_after_male_draft, traits: [:male, :female, :draft]
        factory :male_after_female_draft, traits: [:female, :male, :draft]
      end
  ...
  context "the other child class with one trait" do
    ...
    its(:slug)   { should eq "Female" }
    it           { should_not be_draft }
  end

  context "the child with multiple traits" do
    subject      { FactoryBot.create(:female_draft) }
    its(:name)   { should eq "Jane" }
    its(:slug)   { should eq "Female" }
    it           { should be_draft }
  end
  ...
  context "the child with multiple traits and overridden attributes" do
    subject      { FactoryBot.create(:female_draft, name: "Jill", slug: nil) }
  ...
  context "the child with multiple traits who override the same attribute" do
    context "when the male assigns name after female" do
      subject      { FactoryBot.create(:male_after_female_draft) }
      its(:name)   { should eq "Joe" }
      its(:slug)   { should eq "Male" }
      it           { should be_draft }
    end

    context "when the female assigns name after male" do
      subject      { FactoryBot.create(:female_after_male_draft) }
      its(:name)   { should eq "Jane" }
      its(:slug)   { should eq "Female" }
      it           { should be_draft }
    end
  ...
  context "child class with scoped trait and inherited trait" do
    subject      { FactoryBot.create(:female_draft_judy) }
